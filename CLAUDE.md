...
- `DELETE /api/v1/spending/{id}` — hard delete of a single spending transaction. Spending page table also has a bulk-select checkbox column with a bulk action bar (`#spBulkBar`) to recategorize or delete multiple selected rows at once.
...
## Testing
- Unit tests: `uv run pytest tests/ --ignore=tests/integration --ignore=tests/e2e` (728 passing, 6 skipped)
