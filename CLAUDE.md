...
**Dashboard Portfolio Value History** (`dashNetworthArea` in `index.html`, `loadDashboardNetworthHistory`/`renderDashboardNetworthSparkline` in `pfm_analytics.js`) is a compact sparkline of actual daily snapshots — the same `GET /api/v1/analytics/networth-history` data the Analytics page's full "Net Worth Over Time" chart uses (positions value/cost only; cash isn't tracked historically, hence "Portfolio Value" not "Net Worth" in the card title, matching the KPI card's own scope). Complements the Wealth Simulator card's *projected future* with the *actual past* — placed directly above it. Unlike the Analytics chart (which force-includes a zero baseline for its labelled y-axis), the sparkline scales tightly to the data's own min/max since it has no axis to anchor, so trend variation stays visible; line/fill colour flips green/red on the overall period change sign. Has the same dashed-gridline/`currentColor`-labelled axes treatment as the Wealth Simulator preview chart above (4 y-ticks in compact €, 4 x-ticks as dates via `_fmtEurCompact`/the shared `currentColor` technique). Shows "Not enough history yet" below 2 snapshots, same message as the Analytics page.
...
## Testing
- Unit tests: `uv run pytest tests/ --ignore=tests/integration --ignore=tests/e2e` (1327 passing, 6 skipped); JS: 148 passing
