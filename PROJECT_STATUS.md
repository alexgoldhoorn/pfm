...
**Recent (v2.5.19 follow-up):** **Multi-account CSV export** — `GET /api/v1/export/csv`'s `portfolio_id` param now accepts repeated values (`Optional[List[int]]`), unioning transactions across the selected accounts; Export tab gains a multi-select account list (`#ioExportCsvPortfolios`) that appends repeated `portfolio_id` query params, empty selection still exports everything. 2 new tests in `tests/unit/test_imports_exports.py`.
