...
Some held funds (e.g. `IE0000000011` iShares € Corp Bond, `LU0000000010` Amundi
...
Verified: `IE0000000011.SG` → 5.54 €, `LU0000000010.LU` → 1276.8 € via fast_info
...
under `IE0000000011.SG` is stored under `IE0000000011`. Crypto keeps its
...
`IE0000000011` / `LU0000000010` now get a fresh price and drop off the stale list.
